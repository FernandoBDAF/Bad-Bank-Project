  ...
  return (
    <Layout>
      ...
      <Card
          ...
          <>
            {authenticated ? (
              <>
                <h5>Welcome</h5>
                <p>{loginMessage}</p>
                <Link  to="/" className="btn btn-light">Go to your account</Link>
              </>
            ) : (
            <>
              <h5>Error</h5>
              <p>{loginMessage}</p>
              <button type="submit" className="btn btn-light" onClick={handleTryAgain}>Try again</button>
            </>
            )}
          </>
        ) : (
        ...
        {!authenticated && 
          <>
            <hr style={{ height: '5px', backgroundColor: 'white' }}  className=''/>
            <p class='mt-3'>Do not have an account?</p>
            <Link to="/" className="btn btn-light">Create your account</Link>
          </>
        }
        </>
        }
